...
class BLEMotor:
    ...
    def notify_speed(self, control_signal):
        print(f"Notify control_signal called with control_signal: {control_signal}")  # Debug line
        for conn_handle in self._connections:
            try:
                msg = str(control_signal)
                self._ble.gatts_notify(conn_handle, self._handle, msg)
            except Exception as e:
                print(f"Error notifying control_signal: {e}")  # Catch and print errors
...
while True:
    ...
    for tag in img.find_apriltags():
        ...
        control_signal = [0, 0]  # [steering, distance]

        # Steering control signal based on tag's x-coordinate
        control_signal[0] = tag.cx - img.width() // 2
        control_signal[0] *= -1

        # Calculate distance based on tag size (z_translation)
        # The z_translation gives an estimate of the distance in meters
        control_signal[1] = tag.z_translation * 100  # Convert to cm for easier use

        print("Control signal:", control_signal)

        # Moving left or right based on control_signal[0]
        if -20 < control_signal[0] < 20:
            print("Tag is centered")
        elif control_signal[0] < -20:
